...
def main():
    messages = [x for x in
                        consume_data_points(topic="data_points", group_id="online_learner_app",
    ...
    for msg in messages:
        dataset = pd.concat([dataset, pd.DataFrame([[float(msg.split(",")[0]), float(msg.split(",")[1])]], columns=['x', 'y'])])
        metric.update(float(msg.split(",")[1]),
                      model.predict_one({"x": float(msg.split(",")[0])}))
    ...
    sns.lmplot(x='x', y='y', data=dataset, ci=None, scatter_kws={'color': 'blue'}, line_kws={'color': 'red'})
    ...
    return model.weights['x'], metric.get()
